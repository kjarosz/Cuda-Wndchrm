...
{  double *xcoords,sum;
   ...
   for (row=0;row<image->height;row++)
     for (col=0;col<image->width;col++)
       xcoords[row*image->width+col]=pow((double)(col+1),(double)x);
   ...
   for (col=0;col<image->width;col++)
     ...
     {
        if (y!=0)
        {  if (x==0) xcoords[row*image->width+col]=pow((double)(row+1),(double)y);
           else
           xcoords[row*image->width+col]=pow((double)(col+1),(double)y)*xcoords[row*image->width+col];
        }
